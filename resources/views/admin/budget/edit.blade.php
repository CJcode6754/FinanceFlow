            ...
            <form action="{{ route('budget.update', $budget->id) }}" method="POST" class="space-y-5">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="amount" id="amount"
                        class="w-full px-4 py-3 border rounded-lg text-sm focus:outline-none dark:bg-gray-700 focus:ring-2 focus:ring-emerald-500 @error('amount') border-red-400 @enderror"
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', \Carbon\Carbon::parse($budget->start_date)->format('Y-m-d')) }}"
                        class="w-full px-4 py-3 border rounded-lg text-sm focus:outline-none dark:bg-gray-700 focus:ring-2 focus:ring-emerald-500 @error('start_date') border-red-400 @enderror">
                    @error('start_date')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date', \Carbon\Carbon::parse($budget->end_date)->format('Y-m-d')) }}"
                        class="w-full px-4 py-3 border rounded-lg text-sm focus:outline-none dark:bg-gray-700 focus:ring-2 focus:ring-emerald-500 @error('end_date') border-red-400 @enderror">
                    @error('end_date')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
